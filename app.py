...
def run_backend():
    ...
    # Using python -m jarvis.interface.server or similar if needed,
    # but here we just call the script.
    server_path = os.path.join("jarvis", "interface", "server.py")
...
def launch():
    ...
    subprocess.run(["javac", "-d", os.path.join("jarvis", "agents", "java")] + java_files)
    ...
    webview.create_window('JARVIS HUD', 'http://localhost:5000', width=1200, height=800)
    webview.start()
